...
const App: React.FC = () => {
    ...
    if (!hasFetchedRec.current) {
      ...
      const fetchRec = async () => {
        try {
          ...
          setAiRecommendation("Ready for a cold, nostalgic treat?");
        }
      };
      fetchRec();
    }
  ...
  const handleFindOrder = (input: string, type: 'id' | 'phone') => {
    ...
    if (foundOrder) {
      if (['completed', 'cancelled'].includes(foundOrder.status)) {
        alert("This order is already finalized or cancelled.");
      } else {
  ...
  if (activeOrderID && !isAdmin) {
    ...
    if (currentOrder && !['completed', 'cancelled'].includes(currentOrder.status)) {
      return (
        <div className={`theme-container theme-${theme} min-h-screen w-full`}>
  ...
  return (
    <div className={`theme-container theme-${theme} min-h-screen app-container w-full mx-auto flex flex-col transition-colors relative`}>
      <div className="bg-red-600 text-white text-[10px] font-black py-3 uppercase tracking-widest overflow-hidden relative border-b border-red-500 w-full shrink-0">
        <div className="animate-marquee whitespace-nowrap">
          ✨ NEW FLAVORS JUST LANDED! ✨ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ✨ NEW FLAVORS JUST LANDED! ✨ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ✨ NEW FLAVORS JUST LANDED! ✨
        </div>
      </div>
      ...
      <main className={`flex-1 px-4 sm:px-8 pt-4 w-full ${cartItemCount > 0 ? 'pb-32' : 'pb-20'}`}>
        <div className="flex flex-col items-center w-full">
          <header className="mb-8 text-center w-full flex flex-col items-center">
            <h1 className="font-kampung text-4xl sm:text-5xl md:text-6xl tracking-tighter leading-tight font-black select-none text-gradient-primary mb-10 max-w-lg px-2">
              AIR BATU MALAYSIA / ICE LOLLY MALAYSIA
            </h1>
            
            <div className="flex flex-col gap-3 items-center w-full px-2">
              <div className="bg-slate-500/5 border border-[var(--border-color)] p-6 rounded-[2.5rem] shadow-lg w-full max-w-xs flex flex-col items-center gap-4 transition-colors">
                <div className="text-center">
                  <p className="theme-text-muted text-[10px] font-black uppercase tracking-[0.2em] mb-1 opacity-80">Our Promise</p>
                  <p className="text-[var(--text-color)] text-[11px] font-bold leading-tight">Hand-packed and frozen with love</p>
                </div>
                <div className="w-12 h-px bg-[var(--border-color)] opacity-50"></div>
                <div className="text-center">
                  <p className="theme-text-muted text-[9px] uppercase font-black tracking-widest mb-1 opacity-70">Self Pickup @ Tengah</p>
              ...
              <button onClick={() => setIsFindModalOpen(true)} className="mt-1 flex items-center gap-2 text-[var(--text-muted)] hover:text-pink-400 transition-all px-4 py-2 text-[9px] font-black uppercase tracking-[0.2em]">
          ...
          <section className="mb-8 w-full px-2">
            <div className="relative bg-slate-500/5 border border-[var(--border-color)] rounded-[2.5rem] p-8 shadow-2xl overflow-hidden transition-colors text-center min-h-[120px] flex flex-col justify-center">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-pink-400 mb-3">AI Flavor Guide</h4>
              <p className="text-[var(--text-color)] font-bold text-sm sm:text-base md:text-lg leading-snug italic opacity-90">
                "{aiRecommendation || "Chilling the next batch of nostalgic treats..."}"
              </p>
            </div>
          </section>
          ...
          <section className="w-full px-1">
            <div className="flex flex-col items-center mb-10">
              ...
              <div className="w-16 h-1 bg-gradient-to-r from-red-500 via-yellow-400 to-green-500 mt-3 rounded-full shadow-lg"></div>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:gap-6 w-full max-w-full">
      ...
      <footer className="px-8 py-20 text-center bg-slate-500/5 border-t border-[var(--border-color)] flex flex-col items-center gap-12 transition-colors w-full">
         <div className="space-y-4">
            <h3 className="text-gradient-primary font-kampung text-4xl font-black uppercase tracking-tighter">AIR BATU MALAYSIA</h3>
            <div className="theme-text-muted text-[11px] font-bold uppercase tracking-widest">Collection: Tengah</div>
         </div>
         <div className="space-y-6">
           <p className="text-[14px] font-black uppercase tracking-[0.2em] font-kampung">
             <span className="text-gradient-primary">© 2026 AIR BATU MALAYSIA</span>
           </p>
           <button onClick={() => setIsLoginModalOpen(true)} className="text-[10px] text-slate-500 hover:text-emerald-500 underline uppercase font-bold tracking-widest transition-colors py-1">
             Staff Access
           </button>
         </div>
      </footer>
      ...
      {cartItemCount > 0 && !isAdmin && (
        <div className="fixed bottom-0 inset-x-0 z-40 pointer-events-none pb-[env(safe-area-inset-bottom,0px)]">
          <div className="max-w-[500px] mx-auto w-full px-4 sm:px-0">
            <div className="pointer-events-auto theme-nav backdrop-blur-xl border-t px-6 py-6 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.2)] transition-colors rounded-t-[2.5rem] border-x border-t border-[var(--border-color)]">
              ...
              <button onClick={() => setIsCartOpen(true)} className="bg-[var(--text-color)] text-[var(--bg-color)] font-black py-4 px-8 rounded-2xl flex items-center gap-3 hover:bg-emerald-400 transition-all active:scale-95 shadow-xl group">
                <div className="relative">
                  ...
                  <span className="absolute -top-2 -right-2 bg-pink-500 text-white text-[8px] w-4 h-4 flex items-center justify-center rounded-full border border-[var(--bg-color)] font-black">
                    {cartItemCount}
                  </span>
                </div>
      ...
      <StaffDashboardModal 
        ...
        onClearOrders={() => {
          const activeStatuses: OrderStatus[] = ['pending', 'accepted', 'ready'];
